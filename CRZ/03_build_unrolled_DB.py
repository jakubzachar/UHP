...
import ast
import re
# úprava - odstránenie číslovania pomocou index_col a možnosť odstrániť duplikáty
DB_clean    = pd.read_csv('CRZ_DB_clean.csv', delimiter = '|',index_col=[0])
# úprava - DB_clean = DB_clean.drop_duplicates() - lepšie urobiť manuálne v exceli podla potreby
...
# úprava - doplnenie tak, aby sa neukladali čísla riadkov index = False
DB.to_csv('CRZ_DB_with_supplements_unrolled.csv', sep = '|', index = False)

# úprava - tento script nefunguje úplne správne - dodatky sa opakujú - avšak tento file nie je ďalej využívaný